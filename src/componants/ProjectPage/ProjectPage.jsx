  ...
  const fetchUsersNotInWorkspace = async (workspaceId, projectId) => {
    ...
    try {
      const response = await axios.get(
        `https://localhost:7042/GetAllUsersNotInWorkspascce?WorkspasceID=${workspaceId}&projectId=${projectId}`,
        {
          headers: {
            authorization: `Bearer ${token}`,
          },
      ...
      console.log(
        "this is response from GetAllUsersNotInWorkspascce api " +
          response.data[0]
      );
    } catch (error) {
      console.error(error);
  ...
  const inviteUserToWorkspace = async (projectId, workspaceId) => {
    ...
    try {
      const response = await axios.post(
        `https://localhost:7042/InviteUserInWorkSpasce?projectid=${projectId}&userId=${selectedUser}&WorkSpacseId=${workspaceId}`,
        {
          headers: {
            authorization: `Bearer ${token}`,
          },
    ...
    <div className="Pmain wrapper">
      ...
      <div className="container pMain">
        ...
        <div className="workspaceSect">
          ...
          <button
            type="button"
            className=" ProjectAdd btn addWS  mb-4 me-3"
            ...
            <div key={index} className="col-md-4 mb-4">
              <div className="card styleCard">
                <div className="card-body Pcard">
                      ...
                      <div key={index} className="col-6 mb-3">
                        <div className="card styleCard">
                          <div className="card-body iconUser">
                            <FaUserCircle className="iconUser" size={30} />
                            <h6 className="iconUser">{user.userName}</h6>
                          </div>
                        </div>
                      </div>
                    ))}
                  ...
                  <button
                    onClick={() => handleAddUserToWorkspace(projectId, workspace.workspaceId)}
                  >
                    Add User
                  </button>
                </div>
